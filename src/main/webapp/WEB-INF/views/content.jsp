		...
		<div class="content-box2">
			...
			<c:if test="${list.size() != 0}">
				<c:forEach var="list" items="${list}">
					<c:if test="${empty search || search ne 'True'}">
						<div class="content-box-item2" onclick="location.href='view?idx=${list.idx}&currentPage=${contentsList.currentPage}'">
					</c:if>
					<c:if test="${not empty search && search eq 'True'}">
						<div class="content-box-item2" 
							onclick="location.href='view2?idx=${list.idx}&currentPage=${contentsList.currentPage}&category=${category}&text=${text}&select=${select}&search=${search}'">
					</c:if>
